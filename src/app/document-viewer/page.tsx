...
/**
 * Standalone Document Viewer Page — NO sidebar layout.
 * 
 * Accepts query params:
 *   - sales_order_id: fetches all callsheet attachments for this SO
 *   - sales_order_no: fallback filter if sales_order_id not provided
 *   - attachment_id: view a single specific attachment
 * 
 * Constructs file URLs using: /api/crm/customer-hub/callsheet/file?id={file_id}&filename={name}
 */
export default async function StandaloneDocumentViewerPage(props: {
    ...
    try {
        ...
        if (Object.keys(filterOptions).length > 0) {
            ...
            if (res.ok) {
                ...
                if (json.data && json.data.length > 0) {
                    attachmentsList = json.data
                        .filter((a: { id: number; file_id: string; attachment_name: string }) => a.file_id)
                        .map((a: { id: number; file_id: string; attachment_name: string }) => ({
        ...
        console.error("Failed to load attachments in standalone document viewer:", e);
    }
