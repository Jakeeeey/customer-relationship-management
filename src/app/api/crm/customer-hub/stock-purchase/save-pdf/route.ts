...
export async function POST(req: NextRequest) {
    try {
        ...
        try {
            ...
            console.error("[Stock Purchase Save PDF API] Error parsing invoice_ids:", e);
        ...
        console.log(`[Stock Purchase Save PDF API] Archiving PDF for Invoice IDs: ${invoiceIds.join(", ")} | Receipts: ${receiptNumbers}`);
        ...
        try {
            ...
            console.warn("[Stock Purchase Save PDF API] Failed to decode token:", e);
        }
        ...
        directusFormData.append("title", `Stock Purchase PDF (${receiptNumbers})`);
        ...
        if (!uploadRes.ok) {
            ...
            console.error(`[Stock Purchase Save PDF API] File Upload failed:`, errorText);
        ...
        if (targetFolderId) {
            ...
            }).catch(err => console.warn("[Stock Purchase Save PDF API] Folder patch failed:", err));
        }
        ...
        const archivePromises = invoiceIds.map(async (id, index) => {
            ...
            if (!recordRes.ok) {
                ...
                console.error(`[Stock Purchase Save PDF API] Failed for invoice ${id}:`, error);
        ...
        console.log(`[Stock Purchase Save PDF API] PDF archived for ${createdRecords.length} invoices.`);

        return NextResponse.json({ success: true, record_ids: createdRecords, file_id: fileId, warning: folderWarning });

    } catch (err: unknown) {
        console.error("[Stock Purchase Save PDF API] Catch Error:", err);
