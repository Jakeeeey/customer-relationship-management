    ...
    try {
        ...
        if (action === "products") {
            try {
                ...
                // --- [RETAINED - future use] Branch & Supplier Resolution ---
                // These variables are resolved for potential future re-integration with
                // an inventory service. They are NOT actively used in the product response.
                ...
                if (salesmanId || queryBranchId) {
                    try {
                        // Fallback: resolve branchId from salesman if not passed in query
                        if (!_branchId && salesmanId) {
                            ...
                            if (smRes.ok) {
                                ...
                                if (smData) {
                                    _branchId = smData.branch_code || smData.branch_id;
                                    if (_branchId && typeof _branchId === 'object') {
                                        const obj = _branchId as { id?: number | string; branch_code?: number | string; branch_id?: number | string };
                                        _branchId = obj.branch_code || obj.id || obj.branch_id || null;
                                    }
                                }
                            }
                        }

                        // Resolve supplier shortcut from Directus
                        if (supplierId) {
                            try {
                        ...
                        // Resolve branch code and name from Directus
                        if (_branchId) {
                            if (isNaN(Number(_branchId))) {
                                _branchCodeStr = String(_branchId);
                                try {
                                ...
                                try {
                                    const bRes = await fetch(`${DIRECTUS_URL}/items/branches/${_branchId}?fields=branch_code,branch_name`, { headers: fetchHeaders });
                                    if (bRes.ok) {
                                        const bData = (await bRes.json()).data;
                        ...
                        console.error("[BranchResolution] Exception:", e);
                    }
                }
                // --- [END RETAINED] Branch & Supplier Resolution ---
                ...
                const finalProducts = sellableItems.map((p) => {
                    ...
                    return {
                        ...
                        unit_count: Number(p.unit_of_measurement_count) || 1
                    };
                });
...
export async function POST(req: NextRequest) {
    try {
        ...
        console.log(`\n>>> [POST /create-sales-order-v2] Received request: order_id=${header.order_id || 'NEW'}, items=${items?.length || 0}`);
