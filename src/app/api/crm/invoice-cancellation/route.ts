...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error("🔥 BFF Network Error (Eligible):", err.message);
    return NextResponse.json({ ok: false, message: "BFF Network Error", detail: err.message }, { status: 502 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("🔥 BFF Network Error (Request):", err.message);
    return NextResponse.json({ ok: false, message: "BFF Network Error", detail: err.message }, { status: 502 });
  }
}