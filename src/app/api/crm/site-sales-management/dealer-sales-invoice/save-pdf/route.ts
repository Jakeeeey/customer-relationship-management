...
export async function POST(req: NextRequest) {
    try {
        ...
        try {
            ...
            console.error("[Dealer Save PDF API] Error parsing invoice_ids:", e);
        ...
        console.log(`[Dealer Save PDF API] Archiving PDF for Invoice IDs: ${invoiceIds.join(", ")} | Receipts: ${receiptNumbers}`);
        ...
        try {
            ...
            console.warn("[Dealer Save PDF API] Failed to decode token:", e);
        }
        ...
        directusFormData.append("title", `Dealer Invoice PDF (${receiptNumbers})`);
        ...
        if (!uploadRes.ok) {
            ...
            console.error(`[Dealer Save PDF API] File Upload failed:`, errorText);
        ...
        if (targetFolderId) {
            ...
            }).catch(err => console.warn("[Dealer Save PDF API] Folder patch failed:", err));
        }
        ...
        const archivePromises = invoiceIds.map(async (id, index) => {
            ...
            if (!recordRes.ok) {
                ...
                console.error(`[Dealer Save PDF API] Failed for invoice ${id}:`, error);
        ...
        console.log(`[Dealer Save PDF API] PDF archived for ${createdRecords.length} invoices.`);

        return NextResponse.json({ success: true, record_ids: createdRecords, file_id: fileId, warning: folderWarning });

    } catch (err: unknown) {
        console.error("[Dealer Save PDF API] Catch Error:", err);
