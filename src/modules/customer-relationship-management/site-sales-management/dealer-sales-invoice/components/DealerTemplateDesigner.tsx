...
export const DealerTemplateDesigner: React.FC<Props> = ({ isOpen, onClose, onSave, initialTemplate, isSaving }) => {
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setIsUploading(true);
        try {
            const fileId = await DealerPrintService.uploadFile(file);
    ...
    const handleTableDrag = (e: React.MouseEvent) => {
        ...
        const rect = canvasRef.current.getBoundingClientRect();
        const startY = e.clientY;
        const initialStartY = template.tableSettings?.startY || 0;
        ...
        const onMouseMove = (moveEvent: MouseEvent) => {
            ...
            setTemplate(prev => ({
                ...
                tableSettings: {
                    ...prev.tableSettings!,
                    startY: Math.max(0, Math.min(template.width, initialStartY + dy))
                }
            }));
        };
    ...
    const handleRowHeightDrag = (e: React.MouseEvent) => {
        ...
        const rect = canvasRef.current.getBoundingClientRect();
        const startY = e.clientY;
        const initialRowHeight = template.tableSettings?.rowHeight || 10;
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="!fixed !top-0 !left-0 !translate-x-0 !translate-y-0 !max-w-none !sm:max-w-none !w-screen !h-screen !p-0 !m-0 flex flex-col gap-0 overflow-hidden bg-background border-none rounded-none shadow-none z-[100]">
                ...
                <div className="flex-1 flex overflow-hidden bg-zinc-100 dark:bg-zinc-950">
                    ...
                    <div className="w-96 border-r border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 p-4 space-y-6 overflow-y-auto">
                        ...
                        <section className="space-y-3">
                            ...
                            <div className="space-y-2">
                                {Object.entries(template.fields).map(([key, configVal]) => {
                                    const config = configVal as ORFieldConfig;
                                    return (
                                    <div 
                                        key={key}
                        ...
                        <section className="space-y-3">
                            ...
                            <div className="p-3 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 space-y-3">
                                <div className="space-y-1">
                                    ...
                                    <Input 
                                        type="number" 
                                        value={template.tableSettings?.fontSize || 10} 
                                        onChange={e => setTemplate(prev => ({
                                            ...prev,
                                            tableSettings: prev.tableSettings ? { ...prev.tableSettings, fontSize: Number(e.target.value) } : undefined
                                        }))}
                                        className="h-8 text-xs font-mono"
                                    />
                                </div>
                                <div className="space-y-1">
                                    ...
                                    <div className="flex gap-2">
                                        <Input 
                                            type="number" 
                                            step="0.1"
                                            value={template.tableSettings?.rowHeight || 10} 
                                            onChange={e => setTemplate(prev => ({
                                                ...prev,
                                                tableSettings: prev.tableSettings ? { ...prev.tableSettings, rowHeight: Number(e.target.value) } : undefined
                                            }))}
                                            className="h-8 text-xs font-mono"
                                        />
                                        <div className="flex flex-col gap-0.5">
                                            <Button variant="outline" size="icon" className="h-4 w-7" onClick={() => setTemplate(prev => ({ ...prev, tableSettings: prev.tableSettings ? { ...prev.tableSettings, rowHeight: Number((prev.tableSettings.rowHeight + 0.1).toFixed(2)) } : undefined }))}>+</Button>
                                            <Button variant="outline" size="icon" className="h-4 w-7" onClick={() => setTemplate(prev => ({ ...prev, tableSettings: prev.tableSettings ? { ...prev.tableSettings, rowHeight: Math.max(0, Number((prev.tableSettings.rowHeight - 0.1).toFixed(2))) } : undefined }))}>-</Button>
                                        </div>
                                    </div>
                                </div>
                                <div className="space-y-1">
                                    ...
                                    <div className="flex gap-2">
                                        <Input 
                                            ...
                                            onChange={e => setTemplate(prev => ({
                                                ...prev,
                                                tableSettings: prev.tableSettings ? { ...prev.tableSettings, product_name_width: Number(e.target.value) } : undefined
                                            }))}
                                            className="h-8 text-xs font-mono"
                                        />
                                        <div className="flex flex-col gap-0.5">
                                            <Button variant="outline" size="icon" className="h-4 w-7" onClick={() => setTemplate(prev => ({ ...prev, tableSettings: prev.tableSettings ? { ...prev.tableSettings, product_name_width: (prev.tableSettings.product_name_width ?? 85) + 1 } : undefined }))}>+</Button>
                                            <Button variant="outline" size="icon" className="h-4 w-7" onClick={() => setTemplate(prev => ({ ...prev, tableSettings: prev.tableSettings ? { ...prev.tableSettings, product_name_width: Math.max(0, (prev.tableSettings.product_name_width ?? 85) - 1) } : undefined }))}>-</Button>
                                        </div>
                                    </div>
                    ...
                    <div ref={scrollRef} className="flex-1 bg-zinc-200 dark:bg-zinc-950 flex items-start justify-center overflow-auto relative p-12">
                        ...
                        >
                            {template.backgroundImage && (
                                <Image 
                                    src={DealerPrintService.getImageUrl(template.backgroundImage)} 
                            ...
                            {Object.entries(template.fields).map(([key, configVal]) => {
                                ...
                                <div
                                    ...
                                    style={{
                                        ...
                                        backgroundColor: activeField === key ? 'rgba(59, 130, 246, 0.1)' : 'transparent'
                                    }}
                                >
                                    {key === 'barcode' ? (
                            ...
                            <div 
                                ...
                                style={{ top: `${template.tableSettings?.startY || 0}mm`, height: '4mm', marginTop: '-2mm' }}
                            ...
                            <div 
                                ...
                                style={{ 
                                    top: `${(template.tableSettings?.startY || 0) + (template.tableSettings?.rowHeight || 0)}mm`, 
                                    height: '4mm', 
                                    marginTop: '-2mm' 
                                }}
                            ...
                            >
                                <span className="mr-8 bg-zinc-800 text-white text-[8px] px-1 rounded-sm select-none opacity-0 group-hover:opacity-100 transition-opacity">
                                    Row Height: {template.tableSettings?.rowHeight || 0}mm (Drag to adjust)
                                </span>
                            </div>
                            ...
                            {(() => {
                                ...
                                return [
                                    ...
                                    className="absolute w-full flex items-center pointer-events-none opacity-40 select-none grayscale text-zinc-900"
                                        style={{ 
                                            top: `${(template.tableSettings?.startY || 0) + (idx * (template.tableSettings?.rowHeight || 0))}mm`,
                                            height: `${template.tableSettings?.rowHeight || 0}mm`,
                                            fontFamily: 'monospace',
                                            fontSize: `${template.tableSettings?.fontSize || 10}pt`,
                                            fontWeight: 'bold'
                                        }}
                                    >
                            ...
                            {template.tableSettings?.columns && Object.entries(template.tableSettings.columns).map(([colKey, colConfig]) => (
                                <div
                                    ...
                                    style={{ 
                                        left: `${(colConfig as { x: number }).x}mm`, 
                                        top: `${template.tableSettings?.startY || 0}mm`,
                                        height: `${(template.tableSettings?.rowHeight || 0) * 4}mm`, // Show 4 rows high
                                        width: '6mm',
                                        marginLeft: '-3mm'
                                    }}
                ...
                <DialogFooter className="p-4 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex justify-between sm:justify-between items-center shrink-0">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button 
                            className="h-10 px-8 rounded-xl font-black text-xs uppercase tracking-[0.15em] shadow-lg shadow-primary/20" 
                            onClick={() => onSave(template)}
                            disabled={isSaving}
                        >
                            {isSaving ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Maximize2 className="w-4 h-4 mr-2" />}
