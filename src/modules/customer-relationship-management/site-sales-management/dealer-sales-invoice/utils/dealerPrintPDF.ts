...
import { ReceiptData, ReceiptItem, ORFieldConfig } from '../types/print';
...
const drawBarcodeVector = (doc: jsPDF, text: string, x: number, y: number, config: ORFieldConfig) => {
    ...
    try {
        ...
        // Removed white background rectangle to allow transparency
        // doc.setFillColor(255, 255, 255);
        // doc.rect(x - quietZoneHorizontal, y - quietZoneVertical, totalBarcodeWidth + (quietZoneHorizontal * 2), barcodeHeight + (quietZoneVertical * 2) + (showText ? 5 : 0), 'F');
        ...
        console.error("[DealerPrintPDF] Native barcode failed:", err);
    }
};

const generateOfficialReceipt = async (data: ReceiptData, existingDoc?: jsPDF, hideBackground: boolean = false): Promise<jsPDF> => {
    ...
    doc.setProperties({
        title: `Dealer Sales Invoice - ${data.receipt_no}`,
        subject: 'Dealer Sales Invoice',
        author: 'VOS Web Supply Chain Management',
        keywords: 'receipt, invoice, dealer',
        creator: 'VOS System'
    });

    // Background image is for alignment guide in preview, but can be hidden for final print/download
    if (template?.backgroundImage && !hideBackground) {
        try {
            const imgUrl = DealerPrintService.getImageUrl(template.backgroundImage);
    ...
    const renderField = (key: string, value: string, defaultX: number, defaultY: number, options: { align?: 'left' | 'center' | 'right' } = {}) => {
        const config = template?.fields?.[key] as ORFieldConfig | undefined;
        ...
        if (config) {
            doc.setFont(config.fontFamily as string, config.fontWeight as string);
        ...
        if (maxWidth) {
            ...
            if (scaleX !== 1) {
                try {
                    ...
                    const docWithScale = doc as jsPDF & { scale?: (x: number, y: number) => void };
    ...
    // Use totals from data (header) if available, otherwise compute from items
    const grossTotal = data.gross_amount ?? data.items.reduce((s, i) => s + (i.unit_price * i.qty), 0);
    const discountTotal = data.discount_amount ?? data.items.reduce((s, i) => s + i.discount_amount, 0);
    const netTotal = data.net_amount ?? (grossTotal - discountTotal);
    const vatAmount = data.vat_amount ?? (netTotal - (netTotal / 1.12));
    const vatableSales = netTotal - vatAmount;
    ...
    const fieldValues: Record<string, string> = {
        ...
        terms: data.payment_name.toUpperCase(),
    ...
    if (template?.fields) {
        Object.entries(template.fields).forEach(([key, configVal]) => {
...
const generateThermalReceipt = async (data: ReceiptData, existingDoc?: jsPDF): Promise<jsPDF> => {
    ...
    center("DEALER SALES INVOICE");
    divider('=');
    wrap("Receipt#:", data.receipt_no);
    wrap("PO#:", data.po_no);
    wrap("Salesman:", data.salesman_name);
    ...
    y += 2;
    center("--- THANK YOU ---");
    center(formatToPHT(new Date(), "MM/dd/yyyy"));
    
    return doc;
};

export const generateDealerPrintPDF = async (data: ReceiptData, existingDoc?: jsPDF, hideBackground: boolean = false): Promise<jsPDF> => {
    if (data.is_official) {
        return generateOfficialReceipt(data, existingDoc, hideBackground);
    } else {
