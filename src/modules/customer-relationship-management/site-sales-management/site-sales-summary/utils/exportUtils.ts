...
export const exportToPDF = async (
    ...
    // 2. Initialize jsPDF with the desired orientation
    const unit = 'mm';
    let pdfFormat: string | [number, number] = config?.paperSize?.toLowerCase() || 'a4';
    if (pdfFormat === 'custom' && config?.customSize) {
