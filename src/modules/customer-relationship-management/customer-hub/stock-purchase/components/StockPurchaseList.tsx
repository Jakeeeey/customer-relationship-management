...
import { useStockPurchaseStore } from "../store";
import { 
    ColumnDef, 
    SortingState,
    useReactTable,
...
import { StockPurchaseHeader, Salesman, Customer, SalesType, WorklistFilters } from "../types";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { EmptyPlaceholder } from "@/components/shared/EmptyPlaceholder";

interface StockPurchaseListProps {
    data: StockPurchaseHeader[];
    isLoading: boolean;
    salesmen: Salesman[];
    customers: Customer[];
...
export const StockPurchaseList: React.FC<StockPurchaseListProps> = ({ 
    data, 
    isLoading,
    salesmen,
    customers,
    ...
    useEffect(() => {
        if (customer && customer !== "all") {
            ...
            if (!exists) {
                stockPurchaseProvider.getCustomers(customer, 1, 1)
                    .then(results => {
                        if (results.length > 0) {
    ...
    const columns: ColumnDef<StockPurchaseHeader>[] = [
        {
    ...
    const ActionComponent = (
        <div className="flex gap-2">
            <Button 
                size="sm" 
                className="rounded-lg gap-2 bg-slate-900 hover:bg-slate-800 text-white"
                asChild
            >
                <Link href="/crm/customer-hub/stock-purchase/create-stock-purchase">
                        ...
                        <TableBody>
                            {isLoading && !data?.length ? (
                                ...
                                table.getRowModel().rows.map((row) => (
                                    <TableRow
                                        key={row.id}
                                        onClick={() => router.push(`/crm/customer-hub/stock-purchase/${row.original.invoice_id}`)}
                ...
                <div className="flex items-center justify-between px-2 mt-4">
                    ...
                    <div className="flex items-center space-x-6 lg:space-x-8">
                        ...
                        <div className="flex items-center space-x-2">
                            <Button
                                variant="outline"
                                className="hidden h-8 w-8 p-0 lg:flex rounded-lg border-slate-200 bg-white"
