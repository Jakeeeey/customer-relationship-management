...
export const StockPurchaseDetails: React.FC<StockPurchaseDetailsProps> = ({ id }) => {
    ...
    const handlePrintReceipt = useCallback(async () => {
        if (!pdfFileId) return;
        try {
            const url = `/api/crm/customer-hub/stock-purchase?type=asset&id=${pdfFileId}`;
                    ...
                    <div className="px-6 py-4 bg-white border-t flex items-center justify-end shadow-[0_-4px_20px_rgba(0,0,0,0.03)]">
                        {pdfFileId && (
                            <Button
                                ...
                                className="rounded-2xl h-11 px-6 bg-indigo-600 hover:bg-indigo-700 text-white font-black tracking-tighter active:scale-95 transition-all mr-2 flex items-center gap-2"
                            >
