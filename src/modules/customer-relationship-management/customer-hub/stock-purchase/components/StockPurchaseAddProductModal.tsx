...
import { SearchProduct, StockPurchaseDetail, CartItem } from '../types';
import { calculateChainNetPrice } from '../utils';
import { cn } from '@/lib/utils';
import { toast } from 'sonner';

interface StockPurchaseAddProductModalProps {
    isOpen: boolean;
    onClose: () => void;
    onConfirm: (items: StockPurchaseDetail[]) => void;
    products: SearchProduct[];
    isLoading: boolean;
    initialDetails: StockPurchaseDetail[];
...
export const StockPurchaseAddProductModal: React.FC<StockPurchaseAddProductModalProps> = ({
    isOpen,
    onClose,
    onConfirm,
    products,
        ...
        setCart(prev => prev.map(item => {
            if (item.product_id === productId) {
                ...
                return {
                    ...item,
                    quantity: newQty,
                    unit_discount: currentUnitDiscount,
    ...
    const handleSubmit = () => {
        const details: StockPurchaseDetail[] = cart.map(item => ({
            detail_id: item.detail_id,
            invoice_id: 0,
            product_id: {
                                ...
                                {isLoading ? (
                                    <div className="space-y-4">
                                        ...
                                        {Array(4).fill(0).map((_, i) => (
                                            <div key={i} className="p-5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[28px] space-y-3 shadow-sm opacity-50 animate-pulse">
