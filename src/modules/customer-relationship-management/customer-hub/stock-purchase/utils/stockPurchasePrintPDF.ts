...
const drawBarcodeVector = (doc: jsPDF, text: string, x: number, y: number, config: ORFieldConfig) => {
    ...
    try {
        ...
        console.error("[StockPurchasePrintPDF] Native barcode failed:", err);
    }
};
...
const generateOfficialReceipt = async (data: ReceiptData, existingDoc?: jsPDF, hideBackground: boolean = false): Promise<jsPDF> => {
    ...
    doc.setProperties({
        title: `Stock Purchase - ${data.receipt_no}`,
        subject: 'Stock Purchase',
        author: 'VOS Web CRM',
        keywords: 'receipt, invoice, stock purchase',
        creator: 'VOS System'
    });

    if (template?.backgroundImage && !hideBackground) {
        try {
            const imgData = data.backgroundImageDataUrl || await getImageDataUrl(StockPurchasePrintService.getImageUrl(template.backgroundImage));
...
export const generateStockPurchasePrintPDF = async (data: ReceiptData, existingDoc?: jsPDF, hideBackground: boolean = false): Promise<jsPDF> => {
    if (data.is_official) {
