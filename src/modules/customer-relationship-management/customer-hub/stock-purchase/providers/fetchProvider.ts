...
const API_BASE = "/api/crm/customer-hub/stock-purchase";

const getCache = <T>(key: string): T | null => {
    if (typeof window === "undefined") return null;
    try {
        const item = localStorage.getItem(`cache_sp_${key}`);
...
const setCache = <T>(key: string, data: T): void => {
    ...
    try {
        localStorage.setItem(`cache_sp_${key}`, JSON.stringify(data));
    } catch (e) {
        console.error(e);
    }
};

export const stockPurchaseProvider = {
    getWorklist: async (params: WorklistFilters): Promise<{ data: StockPurchaseHeader[], metadata: { totalCount: number } }> => {
