...
import { Printer, X, Layout, FileText, Eye, RefreshCcw, Download } from "lucide-react"
...
export const InventoryReportPrintModal = ({ 
    ...
    // Update PDF when template or filters change
    useEffect(() => {
        if (isOpen && selectedTemplateName && companyData) {
            handleGeneratePreview();
        }
    }, [selectedTemplateName, companyData]);
    ...
    const handleGeneratePreview = async () => {
        ...
        try {
            const doc = await PdfEngine.generateWithFrame(selectedTemplateName, companyData, (doc, startY, config) => {
                ...
                // Set dynamic filename in metadata
                doc.setProperties({
                ...
                if (filters.mode === "Breakdown") {
                    head = [['BARCODE', 'BRANCH', 'BRAND', 'CATEGORY', 'PRODUCT', 'UNIT', 'AVAILABLE STOCK']];
                    data.forEach(item => {
                    ...
                    head = [['BRANCH', 'BRAND', 'CATEGORY', 'PRODUCT', 'AVAILABLE STOCK']];
                    body = data.map(item => [
                        item.branch,
                        item.brand,
                        item.category,
                ...
                autoTable(doc, {
                    ...
                    columnStyles: filters.mode === "Breakdown" ? {
                        ...
                        1: { cellWidth: 35 }, // BRANCH
                        2: { cellWidth: 15 }, // BRAND
                        3: { cellWidth: 25 }, // CATEGORY
                        4: { cellWidth: 'auto' }, // PRODUCT
                        5: { cellWidth: 18 }, // UNIT
                        6: { cellWidth: 35, halign: 'center' }, // AVAILABLE STOCK
                    } : {
                        0: { cellWidth: 35 }, // BRANCH
                        1: { cellWidth: 20 }, // BRAND
                        2: { cellWidth: 25 }, // CATEGORY
                        3: { cellWidth: 'auto' }, // PRODUCT
                        4: { cellWidth: 35, halign: 'center' }, // AVAILABLE STOCK
                    },
                    didDrawCell: (cellData) => {
                        // Render Barcode in Breakdown mode - Now at index 0
                        if (filters.mode === "Breakdown" && cellData.column.index === 0 && cellData.section === 'body' && cellData.cell.raw) {
                            ...
                            if (barcodeValue && typeof window !== 'undefined') {
                                try {
                                    ...
                                    // Clear text so it doesn't overlap
                                    cellData.cell.text = [""];
                                } catch (e) {
                ...
                // Force page numbers if not already set by template
                if (config) {
                    config.pageNumber = {
                        show: true,
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            {/* INLINE STYLE GINAMIT PARA 100% OVERRIDE ANG DEFAULT WIDTH NG SHADCN */}
            <DialogContent 
            ...
            >
                        ...
                        <div className="flex flex-wrap items-center gap-6 bg-muted/40 p-3 px-6 rounded-[2rem] border shadow-sm flex-1 lg:max-w-fit">
                            <div className="flex flex-col gap-1.5 px-2">
                                ...
                                <Input 
                                    ...
                                    className="h-8 bg-transparent border-none text-base font-black focus:ring-0 ring-0 outline-none min-w-[300px] cursor-pointer text-foreground/80 p-0 shadow-none"
                                />
                            </div>
                ...
                {/* BINAWASAN ANG PADDING (naging p-6) PARA LUMAKI YUNG PDF */}
                        ...
                        <div className="flex items-center gap-8">
                             <div className="flex flex-col items-start gap-0.5">
                                <span className="text-[10px] font-black uppercase text-muted-foreground/60 tracking-widest flex items-center gap-1.5">
                                    <div className="h-1 w-1 rounded-full bg-primary/40" /> Print Scope
                                </span>
