...
interface GenerateParams {
    supplierDataList: { supplierName: string; items: PriceListItem[] }[];
    templateName: string;
    salesmanName: string;
    salesmanCode: string;
    segmentName: string;
...
export async function generatePricePrintablesPDF({
    ...
    return await PdfEngine.generateWithFrame(templateName, companyData, async (doc, initialStartY, config) => {
        ...
        const configMargins = config.margins || { top: 10, bottom: 25, left: 14, right: 14 };
        const margins = {
            top: configMargins.top,
            bottom: configMargins.bottom,
            left: 10, // Enforce left margin
            right: 10 // Enforce right margin
        };
        const pageWidth = doc.internal.pageSize.getWidth();
        const contentWidth = pageWidth - margins.left - margins.right;

        for (let sIdx = 0; sIdx < supplierDataList.length; sIdx++) {
            const { supplierName, items } = supplierDataList[sIdx];

            let startY = initialStartY;
            if (sIdx > 0) {
                doc.addPage();
                startY = await PdfEngine.applyTemplate(doc, templateName, companyData);
            }
            ...
            if (isSerializedLayout) {
                ...
                if (showBarcode) {
                    colStyles[0] = { cellWidth: 22, halign: 'center', fontSize: 7 }; // Barcode
